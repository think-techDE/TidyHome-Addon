...
- `settings_ui.py`, `settings_exports.py` und `settings_admin_ui.py` trennen Personen-Einstellungen, Export-Responses und Admin-Seitenaufbau
...
- Personen-Einstellungs-Karten aus `routes/settings.py` in `settings_ui.py` ausgelagert
- CSV-/JSON-Export-Responses in `settings_exports.py` gebündelt
- Admin-Seitenaufbau aus `routes/settings.py` in `settings_admin_ui.py` ausgelagert
- `routes/settings.py` bleibt für Routen, Rechteprüfung, Formularauswertung und Redirects zuständig
- Syntaxprüfung und vollständige Unit-Test-Suite erneut validiert
...
- Storage-Modultests für Aufgaben, Projekte, Personen/Punkte und `storage.py`-Fassade ergänzt
- Initiale Aufgabenaktionen aus `routes/tasks.py` in `task_actions.py` ausgelagert
- Aufgabenroute hält alte private Helfernamen als Alias kompatibel
- Syntaxprüfung und vollständige Unit-Test-Suite erneut validiert
...
- App-weite Einstellungen aus `storage.py` in `storage_app.py` ausgelagert
- Admins, Raum-Icons und Urlaubsmodus bleiben über `storage.py` kompatibel importierbar
- Syntaxprüfung und vollständige Unit-Test-Suite erneut validiert

### v1.3.58

- Haushaltshilfe-Storage aus `storage.py` in `storage_housekeeping.py` ausgelagert
- TinyDB-Instanz und Datenpfade in `storage_runtime.py` gebündelt
- `storage.py` bleibt als kompatible Import-Fassade erhalten
- Syntaxprüfung und vollständige Unit-Test-Suite erneut validiert
...
- Haushaltshilfe-UI-Helfer aus `routes/housekeeping.py` in `housekeeping_ui.py` ausgelagert
- Formulare, Monatskarten, Zeitlisten, Stundensatz-Historie und Quick-Time-Script strukturell getrennt
- Syntaxprüfung und vollständige Unit-Test-Suite erneut validiert

### v1.3.55

- Haushaltshilfe-Formatierung aus `routes/housekeeping.py` in `housekeeping_format.py` ausgelagert
- CSV-/PDF-Abrechnungsexporte in `housekeeping_exports.py` gebündelt
- Syntaxprüfung und vollständige Unit-Test-Suite erneut validiert
...
- Generische UI-Helfer aus `render.py` in `ui_helpers.py` ausgelagert
- Bestehende Imports aus `render.py` für Routen kompatibel gehalten
- Syntaxprüfung und vollständige Unit-Test-Suite erneut validiert
